        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 text-sm font-medium text-gray-900">₱{{ number_format($bill->total_amount, 2) }}</td>
                    <td class="px-4 py-3 text-sm {{ $bill->penalty_amount > 0 ? 'text-red-600 font-medium' : 'text-gray-400' }}">
                    ...
                    <td class="px-4 py-3"><span class="badge {{ $bill->status_badge }}">{{ ucfirst($bill->status) }}{{ $bill->days_overdue > 0 && $bill->status !== 'paid' ? " ({$bill->days_overdue}d)" : '' }}</span></td>
                    <td class="px-4 py-3 text-right space-x-1">
                        @if($bill->status !== 'paid' && $bill->status !== 'archived')
                            <button wire:click="openPayment({{ $bill->id }})" class="text-xs text-green-600 hover:text-green-800 font-medium">Confirm Pay</button>
                            @if($bill->penalty_amount > 0)
                                <button wire:click="openOverride({{ $bill->id }})" class="text-xs text-amber-600 hover:text-amber-800 font-medium">Override</button>
                            @endif
                        @endif
                    </td>
                </tr>
                ...
                <div>
                    ...
                    <select wire:model="genContractId" class="form-input">
                        ...
                        @foreach($activeContracts as $c)
                            <option value="{{ $c->id }}">{{ $c->tenant->full_name }} — Room {{ $c->room->room_number }} (₱{{ number_format($c->base_rent_rate,2) }}/mo)</option>
                        @endforeach
                    </select>
            ...
            <form wire:submit="recordInitial" class="space-y-4">
                <div>
                    ...
                    <select wire:model="initContractId" class="form-input">
                        <option value="">— Select —</option>
                        @foreach($activeContracts as $c)
                            <option value="{{ $c->id }}">{{ $c->tenant->full_name }} — Room {{ $c->room->room_number }}</option>
                        @endforeach
                    </select>
                </div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6">
            ...
            <form wire:submit="saveOverride" class="space-y-4">
                ...
                <div><label class="form-label">Reason (required) *</label><textarea wire:model="overrideReason" rows="2" class="form-input"></textarea>@error('overrideReason')<p class="text-xs text-red-600 mt-1">{{ $message }}</p>@enderror</div>
                <div class="flex justify-end gap-3 pt-2">
