<div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3"><span class="badge {{ $room->status_badge }}">{{ str_replace('_',' ',ucfirst($room->status)) }}</span></td>
                    ...
                    <td class="px-4 py-3 text-right">
                        <div class="flex items-center justify-end gap-1" x-data="{ open: false }">
                            <button wire:click="edit({{ $room->id }})" class="text-brand-600 hover:text-brand-800 text-xs font-medium">Edit</button>
                            <div class="relative">
                                ...
                                <div x-show="open" @click.away="open = false" class="absolute right-0 mt-1 w-40 bg-white rounded-lg shadow-lg ring-1 ring-gray-200 py-1 z-20">
                                    <button wire:click="updateStatus({{ $room->id }}, 'available')" @click="open=false" class="block w-full text-left px-3 py-1.5 text-xs hover:bg-gray-50 text-green-700">Available</button>
                                    <button wire:click="updateStatus({{ $room->id }}, 'occupied')" @click="open=false" class="block w-full text-left px-3 py-1.5 text-xs hover:bg-gray-50 text-blue-700">Occupied</button>
                                    <button wire:click="updateStatus({{ $room->id }}, 'under_maintenance')" @click="open=false" class="block w-full text-left px-3 py-1.5 text-xs hover:bg-gray-50 text-amber-700">Maintenance</button>
                                </div>
                            </div>
                            <button wire:click="archiveRoom({{ $room->id }})" wire:confirm="Archive this room record?" class="text-gray-400 hover:text-red-600 text-xs">Archive</button>
                        </div>
                    </td>
                </tr>
                @endforeach
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" x-data x-init="$el.focus()">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto p-6" @click.away="$wire.set('showModal', false)">
            <h3 class="text-lg font-semibold mb-4">{{ $editing ? 'Edit Room' : 'Add Room' }}</h3>
                    ...
                    <div class="flex flex-wrap gap-2 mt-1">
                        @foreach($this->availableAmenities as $amenity)
                            <label class="inline-flex items-center gap-1.5 text-sm cursor-pointer">
                                <input type="checkbox" wire:model="amenitiesInput" value="{{ $amenity }}" class="rounded border-gray-300 text-brand-600 focus:ring-brand-500">
                                {{ $amenity }}
                            </label>
                        @endforeach
                    </div>
