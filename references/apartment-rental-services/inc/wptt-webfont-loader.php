...
if ( ! class_exists( 'Apartment_Rental_Services_WPTT_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Apartment_Rental_Services_WPTT_WebFont_Loader {
		...
		public function get_remote_url_contents() {
			...
			$apartment_rental_services_contents = wp_remote_retrieve_body( $response );

			return $apartment_rental_services_contents;
		}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	...
	function wptt_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new Apartment_Rental_Services_WPTT_WebFont_Loader( $url );
